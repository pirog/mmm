/**
 * Contains some helpers to add and parse mmm tasks.
 *
 * mmm tasks are a high level abstraction that should contain the neded
 ...
 *
 * @since 1.0.0
 * @see https://github.com/mmm/mmm/issues/351
 * @example
 *
 * // Gets all the tasks that have been loaded
 * var largv = mmm.tasks.parseGlobals();
 */
...
/**
 * A singleton object that contains the mmm global options.
 *
...
/**
 * Parses a mmm task object into something that can be used by the [yargs](http://yargs.js.org/docs/) CLI.
 *
 * A mmm task object is an abstraction on top of yargs that also contains some
 * metadata about how to interactively ask questions on both the CLI and GUI. While this
 * method is useful, any task added to mmm via `mmm.tasks.add` will automatically
 ...
 * @param {Object} task - A mmm task object (@see add for definition)
...
exports.parseToYargs = function(task) {
  ...
  command.handler = function(argv) {
    ...
    // Get mmm
    var mmm = require('./mmm')(require('./config'));
    var cmd = argv._[0];
     ...
     */
    return mmm.events.emit(answersEvent, {argv: argv, inquirer: inquiry})
    ...
    .then(function(answers) {
       ...
       */
      return mmm.events.emit(runEvent, truth)
...
/**
 * Adds a mmm task to the global `mmm.tasks.task` object.
 *
 * A mmm task object is an abstraction on top of [yargs](http://yargs.js.org/docs/)
 ...
 * @param {Object} task - A mmm task object
