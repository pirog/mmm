 ...
 *
 * Logged entries are printed to the console and to `mmm.log` and `error.log`
 * in `$HOME/.mmm/logs`. The verbosity of these logs is determined by the
 * mmm global config.
 *
 * @since 1.0.0
 * @module log
 ...
 * mmm.log.error('This is an err with details %j', err);
 *
 * // Log a silly message
 * mmm.log.silly('This is probably too much logging!');
 *
 * // Log an info message
 * mmm.log.info('Loaded up the %s app', appName);
 */
 ...
 * mmm.log.warning('Something is up with app %s in directory %s', appName, dir);
 */
/**
 ...
 * mmm.log.verbose('Config file %j loaded from %d', config, directory);
 */
/**
 ...
 *
 * // Log a debug message
 * mmm.log.debug('All details about docker inspect %j', massiveObject);
 */
/**
 ...
 *
 * // Log a silly message
 * mmm.log.silly('All details about all the things', unreasonablySizedObject);
 *
 * // Log a silly message
 * mmm.log.silly('If you are seeing this you have delved too greedily and too deep and likely have awoken something.');
 */
var log = function() {
  ...
  var logger = new winston.Logger({
    transports: [
      ...
      new winston.transports.File({
        ...
        maxSize: 25000000,
        maxFiles: 3,
        filename: path.join(logRoot, 'mmm.log'),
      })
    ],
    exitOnError: true
  });
