 ...
 * return mmm.shell.sh([COMPOSE_EXECUTABLE].concat(cmd), opts);
 *
 * // Determine the location of the 'docker' command
 * var which = mmm.shell.which(DOCKER_EXECUTABLE);
 *
 * // Escape spaces in a command before it is exece
 * var cmd = mmm.shell.escSpaces(opts.entrypoint.join(' '));
 *
 * // Escape a command to make it more cli friendly before it is exece
 * var cmd = mmm.shell.esc(cmd);
 */
 ...
 * return mmm.shell.sh(['ls', '-lsa', '/'], {mode: 'collect'})
 *
 ...
 * var escapedCmd = mmm.shell.escSpaces(['git', 'commit', '-m', 'my message']);
 */
 ...
 *
 * // Escape the cmd
 * var escapedCmd = mmm.shell.esc(['git', 'commit', '-m', 'my message']);
 */
exports.esc = esc;
